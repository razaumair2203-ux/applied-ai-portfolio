...
def check_documentation_structure() -> None:
    ...
    assert root.index("## At a glance") < root.index("## Portfolio by domain")
    assert root.index("## Portfolio by domain") < root.index("## Flagship systems and evidence")
    assert "Officer In Charge Projects / R&D & Systems Engineering Lead" in root
    assert "Core specialization — Applied AI Systems Engineering." in root
    ...
    assert "visuals/autonomy-edge-ai/tir-fod/airborne_platform.png" in root
    assert "visuals/autonomy-edge-ai/clear-run/field_trial_evidence.png" in root
    assert "visuals/compliance-regtech/lodestar/product-surface.svg" in root
    assert "visuals/compliance-regtech/i-msha/dashboard-regression.png" in root
    assert "visuals/practical-ai-products/joblooper/dashboard-surface.svg" in root
    assert "visuals/assurance-digital-engineering/adversarial-review/audit-report-preview.jpg" in root
    assert "visuals/autonomy-edge-ai/counter-uas/phase1_original_mount.jpg" in root
    ...
    assert "../../visuals/autonomy-edge-ai/mbse/clear_run_system_architecture.svg" not in mbse_case
